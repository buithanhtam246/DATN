...
checkout(){
	...
	const selected = (this.selectedShippingMethod || '').toLowerCase();
	const isCod = selected.includes('cod');
	if (selected.includes('vnpay') || selected.includes('vnpay') || selected.includes('vnp')) {
		...
		this.orderService.checkout(orderData).subscribe({
			...
			error: (error: any) => {
				...
				this.notificationService.showError(error.error?.message || 'Không thể tạo đơn hàng trước khi thanh toán VNPay');
			}
		});
		return;
	}
